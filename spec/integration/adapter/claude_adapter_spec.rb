...
RSpec.describe Spurline::Adapters::Claude, :integration do
  let(:adapter) do
    described_class.new(
  ...
  describe "#stream" do
    it "streams text and ends with stop_reason=end_turn" do
      ...
      with_integration_cassette("integration/adapter/simple_text_streaming") do
        adapter.stream(
          messages: [
            {
              role: "user",
    ...
    it "emits tool_start chunks for tool_use responses" do
      ...
      with_integration_cassette("integration/adapter/tool_call_response") do
        adapter.stream(
          messages: [
            {
              role: "user",
    ...
    it "accepts multi-message context with tool schemas in the API payload" do
      messages = [
      ...
      expect(adapter.send(:format_messages, messages)).to eq(messages)
      formatted_tools = adapter.send(:format_tools, tools)
      expect(formatted_tools.first[:name]).to eq("echo")
      expect(formatted_tools.first[:input_schema]).to eq(tools.first[:input_schema])

      chunks = []
      with_integration_cassette("integration/adapter/message_formatting") do
        adapter.stream(
          messages: messages,
    ...
    it "raises AuthenticationError on 401 responses (webmock only)" do
      VCR.turned_off do
        ...
        expect {
          ...
          expect(error.class.name).to eq("Anthropic::Errors::AuthenticationError")
        }
      end
    end
  end
