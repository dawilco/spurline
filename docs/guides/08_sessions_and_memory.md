...
3. **Recalled long-term memories** (trust: `:operator`) -- semantic matches for the current input, when configured.
4. **Recent conversation history** (trust: inherited) -- input/output pairs from the memory window, preserving original trust levels.
5. **Current user input** (trust: `:user`) -- the message being processed now.
